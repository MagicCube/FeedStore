package org.magiccube.common.collection;
...
public abstract class ConcurrentKeyedCollection<K, V> extends CopyOnWriteArrayList<V>
{
	...
	private void _addToMap(V element)
	{
		if (element == null)
		{
			RuntimeException ty = new RuntimeException("Element can not be null.");
			throw ty;
		}
		K key = getKeyOfElement(element);
		if (key == null)
		{
			RuntimeException ty = new RuntimeException("Key can not be null.");
			throw ty;
		}
		if (containsKey(key))
		{
			RuntimeException ty = new RuntimeException("Duplicated key '" + key + "'");
			throw ty;
		}
